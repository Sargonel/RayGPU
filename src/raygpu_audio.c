...
typedef struct MRMusicContext { Wave wave; AudioStream stream; unsigned int cursor; double startTime,pauseTime; float seekSeconds; bool playing,paused; } MRMusicContext;
...
Music LoadMusicStreamFromMemory(const char *fileType,const unsigned char *data,int dataSize){
    Wave wave=LoadWaveFromMemory(fileType,data,dataSize);if(!IsWaveValid(wave))return(Music){0};AudioStream stream=LoadAudioStream(wave.sampleRate,wave.sampleSize,wave.channels);if(!IsAudioStreamValid(stream)){UnloadWave(wave);return(Music){0};}
    MRMusicContext *ctx=MemAlloc(sizeof*ctx);if(!ctx){UnloadAudioStream(stream);UnloadWave(wave);return(Music){0};}memset(ctx,0,sizeof*ctx);ctx->wave=wave;ctx->stream=stream;return(Music){stream,wave.frameCount,true,1,ctx};
}
...
bool IsMusicValid(Music music){MRMusicContext *ctx=(MRMusicContext*)music.ctxData;return ctx&&IsWaveValid(ctx->wave)&&IsAudioStreamValid(ctx->stream);}
void UnloadMusicStream(Music music){if(!IsMusicValid(music))return;MRMusicContext *ctx=music.ctxData;UnloadAudioStream(ctx->stream);UnloadWave(ctx->wave);MemFree(ctx);}
static bool mr_queue_music(MRMusicContext *ctx){if(!ctx||ctx->cursor>=ctx->wave.frameCount)return false;rAudioBuffer *buffer=ctx->stream.buffer;unsigned int capacity=buffer->dataSize/(ctx->wave.channels*(ctx->wave.sampleSize/8)),left=ctx->wave.frameCount-ctx->cursor,frames=left<capacity?left:capacity;const unsigned char *source=(const unsigned char*)ctx->wave.data+(size_t)ctx->cursor*ctx->wave.channels*(ctx->wave.sampleSize/8);UpdateAudioStream(ctx->stream,source,(int)frames);ctx->cursor+=frames;return true;}
void PlayMusicStream(Music music){if(!IsMusicValid(music))return;MRMusicContext *ctx=music.ctxData;StopAudioStream(ctx->stream);ctx->cursor=(unsigned int)(ctx->seekSeconds*ctx->wave.sampleRate);if(ctx->cursor>=ctx->wave.frameCount)ctx->cursor=0;if(!mr_queue_music(ctx))return;PlayAudioStream(ctx->stream);ctx->startTime=GetTime();ctx->playing=true;ctx->paused=false;if(IsAudioStreamProcessed(ctx->stream))mr_queue_music(ctx);}
...
void UpdateMusicStream(Music music){if(!IsMusicValid(music))return;MRMusicContext *ctx=music.ctxData;if(!ctx->playing||ctx->paused)return;if(ctx->cursor>=ctx->wave.frameCount){if(music.looping)ctx->cursor=0;else if(!IsAudioStreamPlaying(ctx->stream)){ctx->playing=false;ctx->seekSeconds=GetMusicTimeLength(music);return;}}if(IsAudioStreamProcessed(ctx->stream))mr_queue_music(ctx);}
void StopMusicStream(Music music){if(!IsMusicValid(music))return;MRMusicContext *ctx=music.ctxData;StopAudioStream(ctx->stream);ctx->seekSeconds=0;ctx->cursor=0;ctx->playing=ctx->paused=false;}
