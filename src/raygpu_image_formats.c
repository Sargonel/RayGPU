/* Extra image formats, mipmaps and image export. Compiled through raygpu.c. */
#ifdef _WIN32
...
static Image mr_load_dds(const unsigned char *data,int size){
    ...
    int blockBytes=fourcc==0x31545844u?8:(fourcc==0x33545844u||fourcc==0x35545844u)?16:0;if(!blockBytes){MemFree(rgba);return(Image){0};}int blocksX=(width+3)/4,blocksY=(height+3)/4;if(left<blocksX*blocksY*blockBytes){MemFree(rgba);return(Image){0};}
...
MR_EXPORT("raygpu_readback_allocate") void *raygpu_readback_allocate(unsigned int requestId,unsigned int size){for(int i=0;i<MR_MAX_READBACKS;i++)if(mr_readbacks[i].id==requestId)return MemAlloc(size);return NULL;}
MR_EXPORT("raygpu_readback_complete") void raygpu_readback_complete(unsigned int requestId,void *pixels,int width,int height){for(int i=0;i<MR_MAX_READBACKS;i++)if(mr_readbacks[i].id==requestId){MRReadbackRequest request=mr_readbacks[i];mr_readbacks[i]=(MRReadbackRequest){0};Image image={pixels,width,height,1,7};if(!mr_queue_readback(image,request.callback,request.userData))MemFree(pixels);return;}MemFree(pixels);}
